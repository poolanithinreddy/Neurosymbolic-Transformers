...
Major redesign from v1. Replaces heuristic constraint-based training with:
  - Multi-task learning: NLI + contradiction detection + evidence relevance
  - R-Drop regularization for variance reduction
  - All-sample contrastive learning (not just 5% as in v1)
  - Hard-example mining for REFUTES/NEI boundary
  - Two-phase schedule (simpler, more effective than 3-phase)
  - Learned recalibration replaces heuristic fusion

Training schedule:
  Phase 1 (Epochs 0-1): NLI + aux heads warmup (lower weights)
  Phase 2 (Epochs 2+):  Full multi-task + R-Drop + contrastive

This is the production training loop for the full FEVER dataset.
"""
...
def _eval_split(model, dataloader, device, use_symbolic=False):
    """Evaluate model on a data split.

    Returns accuracy, ECE, Brier, per-label metrics.
    """
    model.eval()
...
def train_fever_veri_v2(
    ...
    if ev_pct < 40:
        logger.warning(
            ...
            "Build wiki cache: python main.py build-fever-wiki-cache", ev_pct,
        )
    ...
    for epoch in range(epochs):
        ...
        # Phase schedule: warmup aux heads first 2 epochs
                ...
                if eval_every > 0 and global_step % eval_every == 0:
                    ...
                    if dev_acc > best_dev_acc:
                        ...
                        torch.save({
                            ...
                            "attn_pool": _unwrap(model).attn_pool.state_dict(),
        ...
        if dev_acc > best_dev_acc:
            ...
            tokenizer.save_pretrained(ckpt_dir)
            torch.save({
                "epoch": epoch, "best_dev_acc": best_dev_acc,
                "contradiction_head": _unwrap(model).contradiction_head.state_dict(),
                "relevance_head": _unwrap(model).relevance_head.state_dict(),
                "recalibration": _unwrap(model).recalibration.state_dict(),
                "contrastive": _unwrap(model).contrastive.state_dict(),
    ...
    # ── Final evaluation ──────────────────────────────────────
    print(f"\n{'─'*40}")
    print("  Final evaluation on dev set (learned recalibration)")
    print(f"{'─'*40}")
    final_dev_raw = _eval_split(model, dev_loader, device, use_symbolic=False)
    ...
    if use_symbolic_at_inference:
        ...
        print(f"{'─'*40}")
        final_dev = _eval_split(model, dev_loader, device, use_symbolic=True)
    ...
    if "dev_test" in splits and splits["dev_test"]:
        ...
        final_dev_test_raw = _eval_split(model, dev_test_loader, device, use_symbolic=False)
        ...
        if use_symbolic_at_inference:
            final_dev_test = _eval_split(model, dev_test_loader, device, use_symbolic=True)
